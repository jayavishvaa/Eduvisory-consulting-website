...
import { Input, DatePicker, Form, notification } from 'antd';
import 'antd/dist/antd.css';
import Countrydropdown from './dropdown/countryDropdown';
import PhoneInput from '../components/phoneInput';
import { signupHandler } from '../utils/fetchCalls';
...
const ValidatedSignup = () => (
	<Formik
		...
		validationSchema={Yup.object().shape({
			...
			country: Yup.string().required('Country and Region cannot be Empty'),
			...
			confirmpassword: Yup.string().required().oneOf([ Yup.ref('password'), null ], 'Password did not match')
		})}
	>
		{(props) => {
			const {
			...
			return (
				<form onSubmit={handleSubmit}>
					...
					<div style={{ paddingTop: 20 }}>
						{errors.middlename &&
						touched.middlename && <div className="input-error">{errors.middlename}</div>}
					</div>
					...
					<div style={{ paddingTop: 20 }}>
						{errors.phonenumber &&
						touched.phonenumber && <div className="input-error">{errors.phonenumber}</div>}
					</div>
					...
					/>
					<div style={{ paddingTop: 20 }}>
						{errors.country && touched.country && <div className="input-error">{errors.country}</div>}
					</div>
					...
					<div style={{ paddingTop: 20 }}>
						{errors.dateofbirth &&
						errors.dateofbirth && <div className="input-error">{errors.dateofbirth}</div>}
					</div>
					...
					/>
					<div style={{ paddingTop: 20 }}>
						{errors.username && touched.username && <div className="input-error">{errors.username}</div>}
					</div>
					...
					<div style={{ paddingTop: 20 }}>
						...
						touched.confirmpassword && <div className="input-error">{errors.confirmpassword}</div>}
					</div>
