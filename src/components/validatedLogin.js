  ...
  return (
    <Formik
      ...
      onSubmit={async (values) => {
        const { emailExists, isAuthenticated, userId } = await loginHandler(values);
        console.log(userId);
        if (!emailExists) {
    ...
    >
      {(props) => {
        const { values, touched, handleChange, isSubmitting, handleSubmit, errors, handleBlur } = props;
        return (
          <form onSubmit={handleSubmit}>
            ...
            <div className="sign-hover" style={{ padding: 20, fontSize: 20 }}>
              or
              <Link to="/signup" style={{ paddingLeft: 20, color: 'black', fontSize: 14 }}>
                SIGN UP
              </Link>
            </div>
          </form>
